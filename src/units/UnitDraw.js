...
function goToShootFrame() {
  ...
  const frameId =
    this.getRotationFrame() * framesPeriods.SHOOT.length +
    framesPeriods.SHOOT.first //12 - index of first frame, 5 - length of one animation loop
  ...
  switch (this.state) {
    ...
    case STATE.SHOOT: {
      const frameId =
        ...
        framesPeriods.SHOOT.first //12 - index of first frame, 5 - length of one animation loop
      if (
        movieClip.currentFrame < frameId ||
        movieClip.currentFrame > frameId + (framesPeriods.SHOOT.length - 1)
      ) {
        movieClip.gotoAndStop(frameId)
      }
      break
    ...
    case STATE.GO: {
      const frameId =
        this.getRotationFrame() * framesPeriods.GO.length +
        framesPeriods.GO.first
      // Impossible
      if (
        movieClip.currentFrame < framesPeriods.GO.first ||
        movieClip.currentFrame > framesPeriods.GO.last
      ) {
        ...
        movieClip.currentFrame < frameId ||
        movieClip.currentFrame > frameId + (framesPeriods.GO.length - 1)
      ) {
        movieClip.gotoAndPlay(frameId)
    ...
    case STATE.FLY: {
      const frameId =
        this.getRotationFrameSmall() * framesPeriods.FLY.length +
        framesPeriods.FLY.first
      if (
        movieClip.currentFrame < frameId ||
        movieClip.currentFrame > frameId + (framesPeriods.FLY.length - 1)
      ) {
        movieClip.animationSpeed = 0.3
        movieClip.gotoAndPlay(frameId)
      } else if (
        movieClip.currentFrame >=
        frameId + (framesPeriods.FLY.length - 1)
      ) {
    ...
    case STATE.GETUP: {
      const frameId =
        this.getRotationFrameSmall() * framesPeriods.GETUP.length +
        framesPeriods.GETUP.first
      if (
        movieClip.currentFrame < frameId ||
        movieClip.currentFrame > frameId + (framesPeriods.GETUP.length - 1)
      ) {
        movieClip.gotoAndPlay(frameId)
      } else if (
        movieClip.currentFrame >=
        frameId + (framesPeriods.GETUP.length - 1)
      ) {
    ...
    case STATE.ABILITY: {
      const frameId =
        this.getRotationFrame() * framesPeriods.GO.length +
        framesPeriods.GO.first +
        7 // because 7th frame looks the best for it
      if (movieClip.currentFrame !== frameId) {
        movieClip.gotoAndStop(frameId)
      }
      break
