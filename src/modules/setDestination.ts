...
const isMoreThanOneWeaponType = (units: Unit[]): boolean => {
  ...
  return units.reduce(
    (prev, curr) => (firstWeapon !== curr.weapon.range ? true : prev),
    false,
  )
}
...
const setDestinationPoint = (
  ...
  if (attack) {
    ...
    units.forEach(unit => {
      // if distance < weapon.range, then unit can stay
    ...
    if (!onePlace) {
      ...
      unitsOutOfRange.map(unit => {
        ...
        if (
          Math.abs(groupX - unit.x) > 200 ||
          Math.abs(groupY - unit.y) > 200
        ) {
      ...
      if (!allInOneGroup) {
        ...
        if (countOther > 1) {
          ...
          unitsInDifferentPlaces.map(place =>
            setDestinationPoint(destination, place, true, false, true),
          )
          return
        }
      }
  ...
  let offsetX =
    unitsCenter.x -
    ('center' in destination ? destination.center.x : destination.x)
  let offsetY =
    unitsCenter.y -
    ('center' in destination ? destination.center.y : destination.y)
  ...
  if (attack) {
    ...
    const d =
      Math.sqrt(
        Math.pow(unitsWithoutPoint[0].weapon.range, 2) -
          Math.pow(positionData.width / 2, 2),
      ) -
      positionData.height / 2
    const newPointX = Math.sin(mainAngle) * -d + destination.center.x
    const newPointY = -Math.cos(mainAngle) * -d + destination.center.y
  ...
  if (positionData.points.length === positionData.cols * positionData.rows) {
    inBoundary(
      positionData.points[positionData.points.length - positionData.cols],
    )
  ...
  //bajzlu na planszy
  positionData.points.forEach(point => {
    //obliczamy odległsoc punktów od jednostek
