...
const getUserUnits = () => {
  return window.allSquads[USER_FACTION].reduce(
    (result, squad) => result.concat(squad.members),
    [] as Unit[],
  )
}
...
const getEnemyUnits = () => {
  return window.allSquads.reduce((result, factionSquads, index) => {
    ...
    const allFactionUnits = factionSquads.reduce(
      (result, squad) => result.concat(squad.members),
      [],
    )
...
export default (function() {
  ...
  const init = () => {
    //only this one function is public
  ...
  const mouseDown = e => {
    ...
    if (window.allSelectedUnits.length > 0) {
      ...
      window.allSelectedUnits = window.allSelectedUnits.filter(
        unit => unit.hp > 0,
      )
    }
    ...
    if (e.which === MOUSE_RIGHT_BUTTON) {
      ...
      if (window.allSelectedUnits.length > 0) {
        ...
        for (let i = 0; i < allEnemyUnits.length; i++) {
          ...
          if (
            Utils.dis(startSelectingPoint, unit) <=
            unit.radius + MOUSE_CLICK_TOLERANCE
          ) {
            const { squad } = unit
            squad.selectAll()
          ...
          if (time === 0) {
            ...
            window.allSquads[0].map((squad: Squad) => {
              if (squad.isAllSelected) {
                const hasAbility = squad.abilities.find(
                  abili => abili.name === name && abili.time === 0,
                )
                if (hasAbility) {
                  squads.push(squad)
                }
      ...
      if (selectedAbility) {
        selectedAbility.squads = selectedAbility.squads.filter(
      ...
      window.allSelectedUnits.map(unit => unit.squad.deselectAll())
      window.allSelectedUnits = []
    }
  }
  ...
  const startSelect = throttle(e => {
    if (selectingRect) {
      ...
      selectingRect.drawRect(
        startSelectingPoint.x,
      ...
      while (i--) {
        ...
        if (unit.x > minX && unit.x < maxX && unit.y > minY && unit.y < maxY) {
          ...
          unit.selected = false
          unit.draw()
          window.allSelectedUnits.splice(
            window.allSelectedUnits.indexOf(unit),
            1,
          )
        }
      }
  ...
  function updateStage() {
    if (
      modX !== 0 &&
      ViewPoint.x + modX >= 0 &&
      ViewPoint.x + modX <= window.mapWidth - window.innerWidth
    ) {
      ViewPoint.x += modX
      window.app.stage.x = -ViewPoint.x
    }
    ...
    if (
      modY !== 0 &&
      ViewPoint.y + modY >= 0 &&
      ViewPoint.y + modY <= window.mapHeight - window.innerHeight
    ) {
      ViewPoint.y += modY
