...
class EffectsFactory {
    ...
    static createBoomEffect(x, y) {
        if(!EffectsFactory.fireEffectFrames) {
            ...
            for(let i = 0; i < 7; i++) {
                const texture = `fireEffect_${i}.png`;
                EffectsFactory.fireEffectFrames.push(PIXI.Texture.from(texture));
            }
        }
    ...
    static createLaserEffect(x, y) {
        if(!EffectsFactory.laserEffectFrames) {
            ...
            for(let i = 0; i < 7; i++) {
                const texture = `laserEffect_${i}.png`;
                EffectsFactory.laserEffectFrames.push(PIXI.Texture.from(texture));
            }
        }
    ...
    static createFlamerEffect(unit) {
        ...
        const start = () => {
            originFlames.map((sprite, index) => {
                if(sprite.alpha < 0.05) {
                    ...
                    const positionOffset = unit.angle + Math.sin((index / flamesCount) * Math.PI * 2 ) / 8;
        ...
        const finish = () => {
            ...
            window.flamesUpdaters = window.flamesUpdaters.filter(updater => updater !== updateFlames);
        }
    ...
    static createPortalEffect(x, y) {
        if(!EffectsFactory.portalEffectFrames) {
            ...
            for(let i = 0; i < 97; i++) {
                ...
                const texture = `Gate_FX_000${index}.png`;
                EffectsFactory.portalEffectFrames.push(PIXI.Texture.from(texture));
            }
        }
