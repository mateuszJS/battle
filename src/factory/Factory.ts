...
class Factory {
  ...
  private timeToCreate: number
  private graphics: PIXI.Sprite
  private _resources: number
  private productionList: IProduction[]
  private productionAnimation: any

  constructor(faction: number, x: number, y: number, angle: number, sortingLayer: PIXI.display.Group) {
  ...
  update() {
    ...
    this.productionList = this.productionList.filter((production => {
      if (++production.progress === production.totalTime) {
        this.createSquad(production.type);
