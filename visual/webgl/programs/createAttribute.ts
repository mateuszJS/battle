export function createAttribute(program: WebGLProgram, name: string) {
  const gl = window.gl
  ...
  if (!arrayBufferLocation) {
    throw Error(
      "gl.createBuffer return null! Probably WebGL context is lost!"
    );
  }
...
export function createAttrIndex() {
  const gl = window.gl
