...
const drawRailingLine = (startPoint, endPoint, offsetAngle) => {
  ...
  for (let i = 0; i <= numberOfRailings; i ++) {
...
const drawBridge = (points: Point[]) => {
  ...
  if (sidePoints === null) { // is a vertical bridge
    ...
    drawRailingLine(sidePoints[0], sidePoints[1], Math.atan2(secondSidePointAvg.x - sidePointAvg.x, sidePointAvg.y - secondSidePointAvg.y))
    drawRailingLine(secondSide[0], secondSide[1], Math.atan2(sidePointAvg.x - secondSidePointAvg.x, secondSidePointAvg.y - sidePointAvg.y))
  }
