...
export function createTexture2dArrayFromSources(device: GPUDevice, sourceList: TextureSlice[], options: Options = {}) {
  const { width, height } = sourceList[0].img
  ...
  sourceList.forEach((texSlice, sliceIndex) => {
    device.queue.copyExternalImageToTexture(
      { source: texSlice.img },
      { texture: textue2dArray, origin: { z: sliceIndex }, mipLevel: 0 },
      { width: texSlice.img.width, height: texSlice.img.height },
    );
