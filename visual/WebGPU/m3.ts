...
const mat3 = {
  fromMat4(m: number[] | Float32Array, dst = new Float32Array(12)) {
  ...
  identity(dst: Matrix = new Float32Array(12)) {
  ...
  multiply(a: Matrix, b: Matrix, dst: Matrix = new Float32Array(12)) {
    const a00 = a[0 * 4 + 0];
    const a01 = a[0 * 4 + 1];
    const a02 = a[0 * 4 + 2];
    const a10 = a[1 * 4 + 0];
  ...
  translation([tx, ty]: [number, number], dst: Matrix = new Float32Array(12)) {
    dst[0] = 1;   dst[1] = 0;   dst[2] = 0;
    dst[4] = 0;   dst[5] = 1;   dst[6] = 0;
  ...
  rotation(angleInRadians: number, dst: Matrix = new Float32Array(12)) {
  ...
  translate(m: Matrix, translation: [number, number], dst: Matrix) {
    return mat3.multiply(m, mat3.translation(translation), dst);
  },

  rotate(m: Matrix, angleInRadians: number, dst: Matrix) {
    return mat3.multiply(m, mat3.rotation(angleInRadians), dst);
  },

  scale(m: Matrix, scale: [number, number], dst: Matrix) {
