...
export default function getProgram(
  ...
  const timeValue = uniformValues.subarray(kTimeOffset, kTimeOffset + 4);

  return function drawTexture(
    pass: GPURenderPassEncoder,
    worldProjectionMatrix: Float32Array,
    vertexData: Float32Array<ArrayBufferLike>,
    perlinTexture: GPUTexture,
    voronoidTexture: GPUTexture,
    time: number,
  ) {
  ...
  const bindGroup = device.createBindGroup({
    ...
    entries: [
      ...
      { binding: 2, resource: perlinTexture.createView() },
      { binding: 3, resource: voronoidTexture.createView() },
    ],
  });
    ...
    timeValue.set(new Float32Array([time, 0, 0, 0]))
