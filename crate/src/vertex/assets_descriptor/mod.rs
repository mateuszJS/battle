...
use crate::{constants::State, SetupFrameDetails};
use std::{
    collections::HashMap,
    sync::{LazyLock, Mutex},
};
...
lazy_static! {
    static ref ASSETS_DESCRIPTOR: HashMap<AssetId, HashMap<State, AnimationDetails>> = {
...
// fn static_assets_descriptor() -> &'static HashMap<AssetId, HashMap<State, AnimationDetails>> {
//     lazy_static! {
//         static ref ASSETS_DESCRIPTOR: HashMap<AssetId, HashMap<State, AnimationDetails>> = {
//             let mut output = HashMap::from([
//                 (AssetId::ElephantHead, elephant_head::get()),
...
pub fn get_asset_descriptor(asset_id: &AssetId, state: &State) -> &'static AnimationDetails {
    let asset = ASSETS_DESCRIPTOR.get(asset_id).unwrap();
    match asset.get(&State::DEFAULT) {
