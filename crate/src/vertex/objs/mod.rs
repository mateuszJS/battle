    ...
    match &OBJS.get(obj_type) {
        Some(verticies) => {
            ...
            while i < verticies.len() {
                ...
                buffer.push(position_angle.cos() * verticies[i + 1] + x); // vertex position
                buffer.push(verticies[i + 2]);
                buffer.push(-position_angle.sin() * verticies[i + 1] + y);
                buffer.push(1.0);
                buffer.push(verticies[i + 3]); //tex source uv
                buffer.push(verticies[i + 4]);
                buffer.push(verticies[i + 5]); //tex layer index
                buffer.push(verticies[i + 6]); //color matrix index
                buffer.push(norm_angle.cos() * verticies[i + 8]); //normals
...
fn add_pick_obj(buffer: &mut Vec<f32>, obj_type: &ObjType, angle: f32, x: f32, y: f32, time: f32) {
    /*
    SELF TIME
    1.33ms - 1.44ms
    // append is my 0.3ms better than extend
