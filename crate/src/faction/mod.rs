...
use crate::Factory;
use crate::World;
pub use ai::{ArtificialIntelligence, FactionInfo, Plan, PurposeType};
...
pub struct Faction {
    pub id: u32,
    pub resources: u32,
    pub squads: Vec<Squad>, // call borrow() and share that Ref<Squad>, if it's not possible then wrap in Rc like Vec<Rc<RefCell<Squad>>>
    pub factory: Factory,
...
impl Faction {
    ...
    fn update_squads_during_creation(&mut self, _world: &mut World) {
        // let factory = &mut self.factory;
        // let mut squad_index: usize = MAX_NUMBER_ITEMS_IN_PRODUCTION_LINE;

        // self.squads_during_creation
        //     .iter_mut()
        //     .enumerate()
        ...
        //             if creating_squad.squad.members.len()
        //                 == creating_squad.squad.squad_details.members_number
        //             {
        //                 squad_index = index;
        //             } else {
        //                 creating_squad.time_to_create_another_unit = 0;
        ...
        for (_key, (sum_x, sum_y, ability_radius, squad_ids)) in group_by_ability.iter() {
            ...
            for (x, y) in track.iter() {
                ...
                if distance > *ability_radius {
                    ...
                    loop {
                        let new_x =
                            last_visited_point.0 + angle_from_last_visited_point.sin() * 100.0;
                        let new_y =
                            last_visited_point.1 - angle_from_last_visited_point.cos() * 100.0;
    ...
    fn ai_attack_closest_enemies(&mut self, plan: Plan) {
        ...
        //         }
        //         None => {
        //             group_by_closest_enemies
        //                 .insert(closest_enemy_id, (closest_weak_enemy, vec![*squad_id]));
        //         }
        //     };
        // });
