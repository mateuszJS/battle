...
use position_utils::map_terrain;
...
use faction::{ArtificialIntelligence, Faction, FactionInfo, PurposeType};
use factory::Factory;
use squad::Squad;
...
#[wasm_bindgen]
impl Universe {
    pub fn new(raw_input: JsValue) -> Universe {
        let input: UniverseInput = match raw_input.into_serde() {
            Ok(data) => data,
            Err(error) => {
                err!("Incorrect structure of input while callign Universe.new");
            }
        };
        let UniverseInput {
            shapes,
    ...
    //                     });
    //                     if is_in_selection {
    //                         read_squad.members.iter().for_each(|squad_member| {
    //                             selected_units_ids.push(squad_member.borrow().id)
    //                         });
    ...
    //                 let option_strategic_point =
    //                     world.strategic_points.iter().find(|strategic_point| {
    //                         let (x, y) = strategic_point.squad.borrow().shared.center_point;
    ...
    //                 && (squad.squad_details.representation_type - ability_type).abs()
    //                     < std::f32::EPSILON
    ...
    //         .iter()
    //         .flat_map(|neighbor| {
    //             Universe::get_all_neightbours(
    //                 neighbor.borrow().shared.center_point,
    //                 not_checked_yet,
    //             )
    //         })
    ...
    //         } else {
    //             let mut squad =
    //                 Squad::new(faction_id as u32, value.round() as u32, SquadType::Solider);
    ...
    pub fn init_frame_descriptor(raw_frames: JsValue) {
        ...
        let frames: Vec<SetupFrameDetails> = if serde.is_ok() {
            serde.unwrap()
        } else {
