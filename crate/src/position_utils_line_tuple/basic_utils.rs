...
pub type Line<'a> = (&'a Point, &'a Point);
...
impl BasicUtils {
  ...
  fn on_line(line: &Line, point: &Point) -> bool {
    ...
    point.x <= line.0.x.max(line.1.x)
      ...
      && (point.y <= line.0.y.max(line.1.y) && point.y <= line.0.y.min(line.1.y))
  }
  ...
  pub fn check_intersection(l1: &Line, l2: &Line) -> bool {
    let dir1: u8 = BasicUtils::direction(l1.0, l1.1, l2.0);
    let dir2: u8 = BasicUtils::direction(l1.0, l1.1, l2.1);
    let dir3: u8 = BasicUtils::direction(l2.0, l2.1, l1.0);
    let dir4: u8 = BasicUtils::direction(l2.0, l2.1, l1.1);
