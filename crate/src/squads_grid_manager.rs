...
impl SquadsGridManager {
    ...
    fn get_indexes_in_area(raw_x: f32, raw_y: f32, raw_radius: f32) -> Vec<usize> {
        let radius =
            (NORMAL_SQUAD_RADIUS + raw_radius + THRESHOLD_MAX_UNIT_DISTANCE_FROM_SQUAD_CENTER)
                * GRID_MAP_SCALE_AVG;
    ...
    pub fn get_squads_in_area_debug(
        _grid: &SquadsGrid, // I think we dont need that parameter, seems like
        raw_x: f32,
        raw_y: f32,
        raw_radius: f32,
    ) -> Vec<f32> {
        ...
        for x in min_x..=max_x {
            for y in min_y..=max_y {
                ...
                let _index = y * GRID_MAP_WIDTH + x; // seems liek we dont need this
            }
        }
