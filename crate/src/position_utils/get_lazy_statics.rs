...
impl GetLazyStatics {
  ...
  pub fn get_obstacles_points() -> Vec<Point>{
    RAW_POINTS
      .iter()
      .map(|(x, y)| Point {
  ...
  pub fn get_obstacles_lines(points: &'static Vec<Point>) -> Vec<Line<'static>> {
    ...
    points
      ...
      .map(|(index, point)| {
        ...
        Line {
          p1: point,
          p2: &points[connected_point_index],
        }
      })
      .collect()
  }

  pub fn get_permanent_connection_graph(
    obstacles_points: &'static Vec<Point>,
    obstacles_lines: &'static Vec<Line>,
  ) -> HashMap<u32, Vec<&'static Point>> {
