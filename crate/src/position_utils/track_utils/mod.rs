...
use super::map_terrain::{
    TERRAIN_LINES, TERRAIN_PERMANENT_DIRECT_CONNECTIONS_GRAPH, TERRAIN_POINTS,
};
...
impl TrackUtils {
    fn calc_complicated_track<'a>(
        start_point: &'a Point,
        end_point: &'a Point,
        obstacles_points: Vec<&'static Point>,
        ...
        [&start_point, &end_point].iter().for_each(|track_point| {
            obstacles_points.iter().for_each(|obstacle_point| {
                ...
                let is_direct_connected = !obstacles_lines.iter().any(|obstacle_line| {
                    obstacle_line.p1.id != obstacle_point.id
    ...
    pub fn calculate_track<'a>(start_point: &'a Point, end_point: &'a Point) -> Vec<&'a Point> {
        ...
        if is_possible_direct_connection {
            ...
            TrackUtils::calc_complicated_track(
                start_point,
                end_point,
                TERRAIN_POINTS.iter().flatten().collect(),
                obstacles_lines,
                &TERRAIN_PERMANENT_DIRECT_CONNECTIONS_GRAPH,
            )
        }
    }
            ...
            .map(|(index, point)| {
                if index == 0 || index == last_index {
                    ...
                    */
                    ...
                    let from_next_point_angle =
                        (point.x - next_point.x).atan2(next_point.y - point.y);
                        ...
                        <= std::f32::EPSILON
                    {
                        ...
                        let sin_mean =
                            (from_previous_point_angle.sin() + from_next_point_angle.sin()) / 2.0;
                        let cos_mean =
                            (from_previous_point_angle.cos() + from_next_point_angle.cos()) / 2.0;
