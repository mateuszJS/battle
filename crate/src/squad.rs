use crate::constants::{State, MAX_SQUAD_SPREAD_FROM_CENTER_RADIUS};
...
impl Squad {
    ...
    pub fn update_center(&mut self) {
        ...
        let (sum_x, sum_y) = members.iter().fold((0.0, 0.0), |(sum_x, sum_y), ref_unit| {
        ...
        if shared.center_point.0.is_nan() || shared.center_point.1.is_nan() {
            ...
            members.iter().for_each(|ref_unit| {
                let unit = ref_unit.borrow();
    ...
    pub fn update(&mut self, world: &mut World) {
        ...
        members.iter_mut().for_each(|ref_unit| {
            ref_unit
                .borrow_mut()
                .update(shared, &mut world.bullets_manager)
        });
    ...
    fn recalculate_members_positions(&mut self) {
        ...
        positions_list
            ...
            .for_each(|(index, position)| {
                let unit: &mut Rc<RefCell<Unit>> = &mut self.members[index];
                unit.borrow_mut()
    ...
    fn add_target(&mut self, destination: (f32, f32), keep_aim_and_ability_target: bool) {
        ...
        self.members.iter_mut().for_each(|ref_unit| {
            ref_unit.borrow_mut().change_state_to_run(shared);
        });
    }
    ...
    pub fn check_units_correctness(&mut self) {
        ...
        if !self.shared.any_unit_started_using_ability {
            ...
            let coherency_not_kept = self.members.iter().any(|ref_unit| {
                let unit = ref_unit.borrow();
    ...
    fn check_members_correctness(&mut self) {
        ...
        members.iter_mut().for_each(|ref_unit| {
            ref_unit.borrow_mut().set_correct_state(shared);
        });
    }
    ...
    fn remove_died_members(&mut self) {
        ...
        self.members
            .retain(|member| member.borrow().state != State::DIE);
    ...
    pub fn is_squad_running(&self) -> Option<f32> {
        ...
        if is_running {
            let (sin_sum, cos_sum) =
                self.members
                    .iter()
                    .fold((0.0, 0.0), |(sin_sum, cos_sum), ref_unit| {
                        let unit = ref_unit.borrow();
                        (sin_sum + unit.angle.sin(), cos_sum + unit.angle.cos())
                    });
            // calc angels mean
