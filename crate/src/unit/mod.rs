...
use crate::constants::{State, MATH_PI};
...
use crate::vertex::{get_asset_descriptor, AnimSpriteConfig, AnimatedSprite, AnimationDetails};
use crate::weapon_types::WeaponType;
use abilities::Abilities;
use std::cell::RefCell;
use std::rc::Rc;
use std::rc::Weak;
use utils::{map_angle_to_index, Utils, FLY_DECELERATION, FLY_MIN_SPEED};
...
impl Unit {
    pub fn new(x: f32, y: f32, squad_details: &'static SquadDetails) -> Unit {
        Unit {
            ...
            sprites: squad_details
                .assets
                .iter()
                .map(|asset_id| AnimatedSprite::new(asset_id))
                .collect(),
    ...
    fn update_sprites_config(&mut self, angle_offset: f32, reset: bool) {
        ...
        sprites.iter_mut().for_each(|sprite| {
            /*
              ...
              State.SHOOT should be also determien by progres <0, 1>!!!
            */
            let AnimationDetails {
                ...
                angles,
                ..
            } = get_asset_descriptor(sprite.asset_id, state);
    ...
    pub fn update_sprites(&mut self, dt: f32, angle_offset: f32, forced: bool) {
        let is_new_state = self.state != self.prev_state;

        if forced || self.angle != self.prev_angle || is_new_state {
            self.prev_angle = self.angle;
            self.prev_state = self.state;
            self.update_sprites_config(angle_offset, is_new_state);
        }

        self.sprites.iter_mut().for_each(|a_sprite| {
            a_sprite.tick(dt);
        })
    }

    pub fn change_state_to_fly(&mut self, angle: f32, strength: f32) {
        if self.state != State::ABILITY {
    ...
    fn set_target(&mut self, x: f32, y: f32) {
        if self.state != State::RUN && self.state != State::CHASING {
    ...
    fn update_run(
        ...
        let is_target_achieved = (self.x - self.target_x).hypot(self.y - self.target_y)
            < self.squad_details.movement_speed;
    ...
    fn change_state_to_shoot(
        ...
        if let Some(ref_cell_unit_aim) = self.aim.upgrade() {
            ...
            if distance <= squad_shared_info.weapon.range {
                self.state = State::SHOOT; // if changed from RUN -> IDLE and still has secondary aim from run
        ...
        if let Some(ref_cell_unit_aim) = nearest_weak_unit_aim.upgrade() {
            ...
            if distance_to_nearest_unit_aim <= squad_shared_info.weapon.range {
                ...
                let distance_to_enemy =
                    squad_shared_info.weapon.range - self.squad_details.movement_speed;
                ...
                if unit_aim.state != State::RUN && unit_aim.state != State::CHASING {
    ...
    fn update_shoot(
        ...
        if self.time_to_next_shoot == 0 {
            ...
            let angle = if self.state == State::SHOOT {
                self.angle
            } else {
    ...
    pub fn update(
        ...
        match self.state {
            State::FLY => self.update_fly(),
            State::GETUP => self.update_getup(squad_shared_info),
            State::RUN | State::CHASING => self.update_run(squad_shared_info, bullet_manager),
            State::SHOOT => self.update_shoot(squad_shared_info, bullet_manager),
            State::ABILITY => Abilities::update_ability(self, squad_shared_info, bullet_manager),
            _ => {}
        }
    }
    ...
    //             State::FLY => self.mod_x.hypot(self.mod_y),
    //             State::GETUP => self.get_upping_progress,
    //             State::SHOOT => self.time_to_next_shoot as f32,
    //             State::ABILITY => Abilities::get_representation_state(self),
    //             State::CHASING => self.get_chasing_representation_param(),
    //             _ => 0.0,
    //         },
    //     ]
    // }
    ...
    pub fn take_damage(&mut self, damage: f32) {
        self.hp -= damage;
        if self.state != State::ABILITY && self.state != State::FLY {
            if self.hp <= 0.0 {
                self.change_state_to_die();
            }
        }
