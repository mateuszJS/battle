use super::{
    BulletsManager, LookUpTable, SquadUnitSharedDataSet, State, Unit, WeaponType,
    RAPTOR_REPRESENTATION_ID, SOLIDER_REPRESENTATION_ID,
};
...
impl Abilities {
    pub fn change_state_to_ability(
        unit: &mut Unit,
        squad_shared_info: &mut SquadUnitSharedDataSet,
    ) {
        if unit.state == State::ABILITY || unit.has_finished_using_ability {
            return;
        }

        unit.state = State::ABILITY;
        // if any unit has state State.ABILITY or has field hasFinishedUsingAbility on true,
